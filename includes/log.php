...
if (!function_exists('start_action')) {
	...
	class Action {
		...
		public function action_log ($columns=80, $time=true) {
			...
			return '* ' . htmlentities(str_pad($this->name, $columns-7)) . static::status_text($this->status) . PHP_EOL .
		...
		public function log_something ($logtext, $symbol='*') {
			if ($symbol === '!') $symbol = '<span class="fail">!</span>';
			else $symbol = htmlentities($symbol, null, 'UTF-8');
			echo '      ' . $symbol . ' ' . htmlentities($logtext, null, 'UTF-8').PHP_EOL;
		}
		...
		private static function status_text ($status) {
			switch ($status) {
				case static::OK   : return '[ <span class="ok">OK</span> ]'; break;
				case static::FAIL : return '[<span class="fail">FAIL</span>]'; break;
			}
		}
